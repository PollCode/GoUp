...
const WhyUs = () => {
  ...
  const cardContent = [
    ...
    {
      title: isSpanish ? "Bienvenido a Go Up Pediatric Extended Care (PPEC) de Miami" : "Welcome to Go Up Pediatric Extended Care (PPEC) of Miami",
      text: isSpanish 
        ...
        <div id="carouselExample" className="carousel slide">
          <div className="carousel-inner">
            {cardContent.map((card, index) => (
              <div className={`carousel-item ${index === 0 ? 'active' : ''} p-2`} key={index}>
          ...
          <button
            ...
            <span className="visually-hidden">{isSpanish ? "Anterior" : "Previous"}</span>
          </button>
          <button
            ...
            <span className="visually-hidden">{isSpanish ? "Siguiente" : "Next"}</span>
          </button>
        </div>
      ) : (
